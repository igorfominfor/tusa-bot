...
async def receive_party(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if photo_id:
            await context.bot.send_photo(chat_id=ADMIN_ID, photo=photo_id, caption=message_text, parse_mode="Markdown")
        else:
...
async def contact(update, context):
    await update.message.reply_text(
        "📩 Написать организатору: @igordotgif"
    )
...
def main():
    app = ApplicationBuilder().token("7885706045:AAFgU3xA9jIJp6i-NiPeWEb00vtWN0j8Tcc").build()
